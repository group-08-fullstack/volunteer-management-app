...
class TestRegister:
    def test_register_success(self,client, user_admin):
        response = client.post("/api/auth/register/", json=user_admin)
        assert response.status_code == 201
        assert response.get_json()["message"] == "Registration successful!"


    def test_register_duplicate(self,client, user_admin, access_token_admin):
    ...
    def test_register_duplicate_unverified(self,client, user_admin,access_token_admin):
    ...
    def test_register_database_error(self,client, user_admin, monkeypatch):
...
class TestLogin:
    ...
    def test_login_unverified_account(self,client, user_admin):
    ...
    def test_login_wrong_password(self,client, user_admin):
    ...
    def test_login_wrong_role(self,client, user_admin):
...
class TestAccess:
    def test_login_database_error(self,client, user_admin, monkeypatch):
    ...
    def test_refresh_token(self,client, user_admin):
        client.post("/api/auth/register/", json=user_admin)
        # Manually insert verficaiton code and set verified to true
        with client.application.app_context():
            conn = get_db()
...
class TestDeleteAccount:
    def test_delete_account_success(self,client, access_token_admin):
    ...
    def test_delete_account_user_not_found(self,client):
    ...
    def test_delete_account_database_error(self,client, access_token_admin, monkeypatch):
...
class TestEmailVerification:
    def test_email_verification_new_email(self,client):
        ...
        with patch('api.auth.smtplib.SMTP_SSL') as mock_smtp:
            ...
            response = client.post("/api/auth/verifyEmail/", json=test_email)
            assert response.status_code == 200
            assert "Verification code sent successfully" in response.get_json()["message"]


    def test_email_verification_existing_email(self,client,access_token_admin,user_admin):
    ...
    def test_email_verification_database_error(self,client, monkeypatch):
    ...
    def test_email_verification_smtp_error(self,client):
...
class TestConfirmation:
    def test_email_code_confirmation_valid(self,client):
        ...
        with patch('api.auth.smtplib.SMTP_SSL') as mock_smtp:
            mock_server = MagicMock()
            mock_smtp.return_value.__enter__.return_value = mock_server
            client.post("/api/auth/verifyEmail/", json=test_email)
        ...
        response = client.post("/api/auth/confirmCode/", json=confirmation_data)
        assert response.status_code == 200
        assert "Verified" in response.get_json()["message"]


    def test_email_code_confirmation_invalid(self,client):
    ...
    def test_email_code_confirmation_database_error(self,client, monkeypatch):
    ...
    def test_email_code_confirmation_update_error(self,client, monkeypatch):
...
class TestEdgecases:
    ...
    def test_edge_case_email_formats(self,client):
       ...
       # Login
        login_response = client.post("/api/auth/login/", json=user_lower)
