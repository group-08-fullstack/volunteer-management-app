...
def test_register_and_login(client, user):
    # Register user
...
def test_register_missing_email(client, user):
...
def test_register_invalid_email_format(client, user):
    invalid_user = dict(user)
    invalid_user["email"] = "invalid-email-format"
    response = client.post("/api/auth/register/", json=invalid_user)    
    assert response.status_code in (201, 400)

def test_register_missing_password(client, user):
    invalid_user = dict(user)
...
def test_register_missing_role(client, user):
    invalid_user = dict(user)
...
def test_login_missing_email(client, user):
    incomplete_user = dict(user)
...
def test_login_missing_role(client, user):
    incomplete_user = dict(user)
...
def test_login_empty_fields(client, user):
...
def test_register_empty_fields(client, user):
